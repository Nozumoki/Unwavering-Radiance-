...
function s.initial_effect(c)
	...
	e1:SetTarget(aux.TargetBoolFunction(Card.IsSetCard,0xF9F))
	c:RegisterEffect(e1)
	--Banish 1 "Unwavering" monster and add 1 "Unwavering" S/T to the hand
	...
	c:RegisterEffect(e2)
	--Return itself and 1 card your opponent controls to the hand
...
function s.desfilter(c)
	return c:IsSetCard(0xF9F) and (c:IsFaceup() or c:IsLocation(LOCATION_HAND))
end
function s.thfilter(c)
	return c:IsSetCard(0xF9F) and c:IsSpellTrap() and c:IsAbleToHand()
end
...
function s.cfilter(c,tp)
	return c:IsFaceup() and c:IsSetCard(0xF9F) and c:IsLocation(LOCATION_SZONE) and not c:IsPreviousLocation(LOCATION_SZONE)
