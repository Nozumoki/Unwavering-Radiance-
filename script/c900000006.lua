...
function s.filter(c)
	return c:IsSetCard(0xF9F) and c:IsFaceup() and c:IsAbleToHand() and not c:IsCode(id)
end
...
function s.spfilter2(c,e,tp)
	return c:IsFaceup() and c:IsSetCard(0xF9F) and not c:IsCode(id) and c:IsCanBeSpecialSummoned(e,0,tp,false,false)
end
function s.sptg(e,tp,eg,ep,ev,re,r,rp,chk,chkc)
	...
	if Duel.IsPlayerAffectedByEffect(tp,CARD_BLUEEYES_SPIRIT) or not Duel.IsEnvironment(900000022) then ct=1 end
	Duel.Hint(HINT_SELECTMSG,tp,HINTMSG_SPSUMMON)
	local g=Duel.SelectTarget(tp,s.spfilter2,tp,LOCATION_REMOVED,0,1,ct,nil,e,tp)
